...
public class Tests
{
    ...
    static void WriteConvertersFile(string solutionDirectory, List<Type> types)
    {
        ...
        foreach (var type in types)
        {
            ...
            var variable = type.Name.ToLowerFirstChar();
            writer.WriteLine(
                $$"""
                               if (parent is {{type.Name}} {{variable}})
                               {
                                   return {{type.Name}}Converter.MemberHasValue({{variable}}, member, value);
                               }
                  """);
        }
